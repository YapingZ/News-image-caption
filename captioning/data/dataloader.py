...
class HybridLoader:
    ...
    def get(self, key, split):
        ...
        if self.in_memory and key in self.features:
            ...
            f_input = open(os.path.join(self.db_path,  key.split('/')[-1].split('.')[0] + self.ext), 'rb').read()
...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.use_spacial = getattr(opt, 'use_spacial', True)
        ...
        print('DataLoader loading news type file: ', opt.input_news_class)
        with open(self.opt.input_news_class,'r', encoding='utf-8') as f:
            news_data = f.readlines()
            self.news_type_list = [news_type.strip() for news_type in news_data ]
        for i, n_t in enumerate(self.news_type_list):
            self.news_type_dict[n_t] = i
        self.news_type_dict['洪水'] = 6
        self.news_type_dict['龙卷风'] = 11
        ...
        self.att_loader = HybridLoader(self.opt.input_att_dir, '.npy', in_memory=self.data_in_memory)
        self.spacial_loader = HybridLoader(self.opt.input_spacial_dir, '.npy', in_memory=self.data_in_memory)
        ...
        for ix in range(len(self.info['images'])):
            ...
            elif img['split'] == 'valid':
                self.split_ix['val'].append(ix)
            elif img['split'] == 'test':
                self.split_ix['test'].append(ix)
        # self.split_ix['val'] += self.split_ix['train'][:int(0.1* len(self.split_ix['train']))]
        # self.split_ix['test'] += self.split_ix['train'][int(0.1 * len(self.split_ix['train'])):int(0.2 * len(self.split_ix['train']))]
        # self.split_ix['train'] = self.split_ix['train'][int(0.2 * len(self.split_ix['train'])):]
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            # fetch image
            tmp_fc, tmp_att, tmp_spacial,tmp_seq, \
            ...
            spacial_batch.append(tmp_spacial.squeeze(0))
            ...
            # info_dict['news_type'] = self.info['images'][ix]['news_type']
            info_dict['file_path'] = self.info['images'][ix].get('file_path', '')
            infos.append(info_dict)
            # a = self.info['images'][ix]['news_type']
            # print(self.info['images'][ix]['news_type'])
            assert self.info['images'][ix]['news_type'] in self.news_type_dict
            news_type.append(self.news_type_dict[self.info['images'][ix]['news_type']])


        # #sort by att_feat length
        # fc_batch, att_batch, label_batch, gts, infos = \
        #     zip(*sorted(zip(fc_batch, att_batch, np.vsplit(label_batch, batch_size), gts, infos), key=lambda x: len(x[1]), reverse=True))
        fc_batch, att_batch, spacial_batch, label_batch, gts, infos = \
            zip(*sorted(zip(fc_batch, att_batch, spacial_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
        data = {}
        ...
        # merge spacial_feats
        # max_spacial_0_len = max([_.shape[0] for _ in spacial_batch])
        # max_spacial_1_len = max([_.shape[1] for _ in spacial_batch])
        data['spacial_feats'] = np.stack(spacial_batch)
        ...
        data['infos'] = infos
        data['news_type'] = np.array(news_type)
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            att_feat = self.att_loader.get(str(self.info['images'][ix]['file_path']),self.info['images'][ix]['split'])
            # Reshape to K x C
            att_feat = att_feat.reshape(-1, att_feat.shape[-1])
            if self.norm_att_feat:
                att_feat = att_feat / np.linalg.norm(att_feat, 2, 1, keepdims=True)
            if self.use_box:
                box_feat = self.box_loader.get(str(self.info['images'][ix]['file_path']),self.info['images'][ix]['split'])
                # devided by image width and height
                x1,y1,x2,y2 = np.hsplit(box_feat, 4)
                h,w = self.info['images'][ix]['height'], self.info['images'][ix]['width']
                # box_feat = np.hstack((x1/w, y1/h, x2/w, y2/h, (x2-x1)*(y2-y1)/(w*h))) # question? x2-x1+1??
                assert w != 0 and h !=0, print('w or h is zero')
                box_feat = np.hstack((x1 / w, y1 / h, x2 / w, y2 / h))
        ...
        if self.use_fc:
            try:
                fc_feat = self.fc_loader.get(str(self.info['images'][ix]['file_path']),self.info['images'][ix]['split'])
            except:
        ...
        if self.use_spacial:
            try:
                spacial_feat = self.spacial_loader.get(str(self.info['images'][ix]['file_path']), self.info['images'][ix]['split'])
            except:
                spacial_feat = np.zeros((0), dtype='float32')
        if hasattr(self, 'h5_label_file'):
            seq = self.get_captions(ix, self.seq_per_img)
        else:
            seq = None
        return (fc_feat,
                att_feat,spacial_feat, seq,
