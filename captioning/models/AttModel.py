...
from ..utils.rewards import get_self_critical_reward_2
from .CaptionModel import CaptionModel
import time
from .utils import Feature2Adj, self_attention, clones, pack_wrapper, LayerNorm, GaussianNet
...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.att_feat_size = opt.att_feat_size - 5
        ...
        # self.prob_embed = nn.Linear(108, self.att_feat_size)
    ...
    def clip_spacial_att(self, spacial_feats, spacial_masks):
        # Clip the length of att_masks and att_feats to the maximum length
        if spacial_masks is not None:
            max_len_x = spacial_masks.data.long().sum(1).max()
            max_len_y = spacial_masks.data.long().sum(2).max()
            spacial_feats = spacial_feats[:, :max_len_x, :max_len_y].contiguous()
            spacial_masks = spacial_masks[:, :max_len_x, :max_len_y].contiguous()
        return spacial_feats, spacial_masks
    ...
    def xyxy2xywh(self, bbox):
        ...
        w = bbox[:,:,2] - bbox[:,:,0]
        h = bbox[:, :, 3] - bbox[:, :, 1]
        out_bbox = torch.cat((x_c.unsqueeze(-1),y_c.unsqueeze(-1),w.unsqueeze(-1),h.unsqueeze(-1)), dim=-1)
        return out_bbox

    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        att_feats, att_masks = self.clip_att(att_feats, att_masks)
        # spacial_feats,  spacial_masks = self.clip_spacial_att(spacial_feats, spacial_masks)
        # embed fc and att feats
        bbox = att_feats[:, :, 1024:]  # [b,n,4]
        att_feats = att_feats[:, :, :1024]  # [b, n, 1024]
        batch_size = att_feats.shape[0]
        fc_feats = self.fc_embed(fc_feats)
        att_feats = pack_wrapper(self.att_embed, att_feats, att_masks)
        query, key = self.q_linear(att_feats), self.k_linear(att_feats)
        adj = self_attention(query, key)  # [batch, n, n]
        mask = torch.where(adj > adj.mean(), torch.full_like(adj, 1), torch.full_like(adj, 0))
        adj = adj * mask
        # adj = self.softmax(adj)
        bbox = self.xyxy2xywh(bbox)
        bbox_copy = bbox.permute(0,2,1).contiguous()
        distance = torch.sqrt((bbox[:,:,1].unsqueeze(-1) - bbox_copy[:,1,:].unsqueeze(1))**2 + (bbox[:,:,0].unsqueeze(-1) - bbox_copy[:,0,:].unsqueeze(1))**2)
        theta = torch.atan((bbox[:,:,1].unsqueeze(-1) - bbox_copy[:,1,:].unsqueeze(1))/(bbox[:,:,0].unsqueeze(-1) - bbox_copy[:,0,:].unsqueeze(1) + 1e-8))
        w_delta = bbox[:,:, 2].unsqueeze(-1) - bbox_copy[:,2, :].unsqueeze(1)
        h_delta = bbox[:,:, 3].unsqueeze(-1) - bbox_copy[:,3, :].unsqueeze(1)
        w_delta = w_delta * mask
        h_delta = h_delta * mask
        theta = theta * mask
        distance = distance * mask
        edge = torch.cat((theta.unsqueeze(-1), distance.unsqueeze(-1), w_delta.unsqueeze(-1), h_delta.unsqueeze(-1)), dim=-1)
        att_gaussion_feats = self.gaussian_net(att_feats, adj, edge)
        # # print(torch.isnan(att_gaussion_feats).int().sum())
        att_feats_2 = att_feats + att_gaussion_feats.view(batch_size, att_feats.shape[1], -1)
        # att_feats_2 = att_feats
        # Project the attention feats first to reduce memory and computation comsumptions.
        p_att_feats = self.ctx2att(att_feats_2)
        # att_feats_2 = att_feats
        # p_att_feats = self.ctx2att(att_feats)
        return fc_feats, att_feats_2, p_att_feats, att_masks, att_feats

    def _forward(self, fc_feats, att_feats, spatial_feats, seq, att_masks=None, news_type=None, spatial_masks=None):
        ...
        outputs = fc_feats.new_zeros(batch_size*seq_per_img, seq.shape[1], self.vocab_size+1)

        # Prepare the features
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, att_feats = self._prepare_feature(fc_feats, att_feats, att_masks)
        logit = self.news_type_classify(fc_feats, spatial_feats, att_feats)
        # pp_att_feats is used for attention, we cache it in advance to reduce computation cost
        if news_type is not None:
            news_type_vector = self.news_type_embed(news_type)
        if seq_per_img > 1:
            p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spatial_feats, p_logit, p_news_type_vector = utils.repeat_tensors(seq_per_img,
                [p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, spatial_feats, logit, news_type_vector]
            )
        pp_logit = p_logit
        for i in range(seq.size(1)):
            ...
            if i >= 1 and seq[:, i].sum() == 0:
                break
            output, state = self.get_logprobs_state(i, it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spatial_feats, pp_logit, p_news_type_vector, state)
            outputs[:, i] = output

        return outputs, logit

    def get_logprobs_state(self, i, it, fc_feats, att_feats, p_att_feats, att_masks, spatial_feats, logit,  p_news_type_vector, state, output_logsoftmax=1):
        # 'it' contains a word index
        # if i == 0:
        #     xt = self.fc(spatial_feats)
        # else:
        xt = self.embed(it)

        output, state = self.core(xt, fc_feats, att_feats, p_att_feats, spatial_feats, logit, p_news_type_vector,  state, att_masks)
    ...
    def _sample_beam(self, fc_feats, att_feats, spacial_feats, att_masks=None, spacial_masks=None,opt={}):
        beam_size = opt.get('beam_size', 10)
        group_size = opt.get('group_size', 1)
        sample_n = opt.get('sample_n', 10)
        # when sample_n == beam_size then each beam is a sample.
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spacial_feats = self._prepare_feature(fc_feats, att_feats,  att_masks,spacial_feats, spacial_masks)
        ...
        logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spacial_feats,state)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spacial_feats = utils.repeat_tensors(beam_size,
            [p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spacial_feats]
        )
        self.done_beams = self.beam_search(state, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,  p_spacial_feats, opt=opt)
        for k in range(batch_size):
            if sample_n == beam_size:
                for _n in range(sample_n):
    ...
    def _sample(self, fc_feats, att_feats, spatial_feats, att_masks=None,  news_type=None, opt={}):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, spatial_feats, att_masks, opt)
        if group_size > 1:
        ...
        if sample_n > 1:
            p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spatial_feats, p_logit, p_news_type_vector = utils.repeat_tensors(sample_n,
                [p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, spatial_feats, logit, news_type_vector]
            )
        else:
            p_logit = logit
            p_spatial_feats = spatial_feats
            p_news_type_vector = news_type_vector
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(t, it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spatial_feats, p_logit, p_news_type_vector, state, output_logsoftmax=output_logsoftmax)
        ...
        return seq, seqLogprobs, p_logit

    def _reward_sample(self, fc_feats, att_feats, spatial_feats,gts, att_masks=None, spatial_masks=None, opt={}):
        sample_method = opt.get('sample_method', 'greedy')
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)
        sample_n = int(opt.get('sample_n', 1))
        ...
        remove_bad_endings = opt.get('remove_bad_endings', 0)
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, spatial_feats, att_masks, spatial_masks, opt)
        if group_size > 1:
            return self._diverse_sample(fc_feats, att_feats, att_masks, opt)

        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size * sample_n)
        logit, spatial_feats = self.news_type_classify(fc_feats, spatial_feats)
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_masks)

        if sample_n > 1:
            p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_spatial_feats, p_logit = utils.repeat_tensors(
                sample_n,
                [p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, spatial_feats, logit]
                )
        else:
            p_logit = logit
            p_spatial_feats = spatial_feats
        ...
        trigrams = []  # will be a list of batch_size dictionaries
        # rewards = fc_feats.new_zeros(batch_size * sample_n, self.seq_length)
        generated_sentence = fc_feats.new_zeros(1, self.seq_length, dtype=torch.int64)
        seq = fc_feats.new_full((batch_size * sample_n, self.seq_length), self.pad_idx, dtype=torch.long)
        seqLogprobs = fc_feats.new_zeros(batch_size * sample_n, self.seq_length, self.vocab_size + 1)
        for t in range(self.seq_length + 1):
            if t == 0:  # input <bos>
                it = fc_feats.new_full([batch_size * sample_n], self.bos_idx, dtype=torch.long)
                current_generated = it.unsqueeze(1)

            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,
                                                      p_spatial_feats, p_logit, state,
            ...
            if t == self.seq_length:  # skip if we achieve maximum length
                break
            it, sampleLogprobs = self.sample_next_word(logprobs, sample_method, temperature)
            current_generated = torch.cat([current_generated, it.unsqueeze(1)], dim=1)
            # start = time.time()
            if t % 6 == 0:
                tmp_generated_sentence = self.rollout_reward(current_generated[:,1:], p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,
                                         p_spatial_feats, p_logit, state,
                                         output_logsoftmax, sample_method, temperature)
                # assert tmp_generated_sentence.size(0) == 800 and tmp_generated_sentence.size(1) == 30
                # print(tmp_generated_sentence.size())
                # print('time of reward {}'.format(time.time() - start))
                generated_sentence = torch.cat((generated_sentence, tmp_generated_sentence), 0)
            # stop when all finished
            if t == 0:
                unfinished = it != self.eos_idx
            else:
        ...
        return seq, seqLogprobs, p_logit, generated_sentence[1:]


    def rollout_reward(self, generated, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,
                                         p_spatial_feats, p_logit, state,
                                         output_logsoftmax, sample_method, temperature,monte_carlo_count=2, steps=1):
        ...
        with torch.no_grad():
            ...
            remaining = self.seq_length - generated.shape[1]

            tmp = [generated, p_fc_feats,p_att_masks, p_spatial_feats,p_logit]
            tmp = [_.repeat(monte_carlo_count, 1) for _ in tmp]
            generated, p_fc_feats,  p_att_masks, p_spatial_feats, p_logit = tmp

            tmp = [p_att_feats, pp_att_feats]
            tmp = [_.repeat(monte_carlo_count, 1, 1) for _ in tmp]
            p_att_feats, pp_att_feats = tmp

            for _ in range(steps):
                state = (state[0].repeat(1, monte_carlo_count, 1), state[1].repeat(1, monte_carlo_count, 1))
                inputs = generated[:,-1]
                current_generated = generated
                for i in range(remaining):
                    logprobs, state = self.get_logprobs_state(inputs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,
                                                              p_spatial_feats, p_logit, state,
                                                              output_logsoftmax=output_logsoftmax)
                    predicted, _ = self.sample_next_word(logprobs, sample_method, temperature)
                    # embed the next inputs, unsqueeze is required cause of shape (batch_size, 1, embedding_size)
                    inputs = predicted
                    current_generated = torch.cat([current_generated, inputs.unsqueeze(1)], dim=1)

                # compute reward

                # reward = get_self_critical_reward_2(gts, current_generated, monte_carlo_count)
                # print('get_self_reward is {}'.format(time.time() - start))
...
# my coding
class UpDown_fusion_Core(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(UpDown_fusion_Core, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, spacial_feats, state, att_masks=None):
        prev_h = state[0][-1]
        att_lstm_input = torch.cat([prev_h, fc_feats, xt], 1)
        ...
        lang_lstm_input = torch.cat([att, h_att, spacial_feats], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
...
class UpDown_type_fusion_Core(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(UpDown_type_fusion_Core, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2  + 512, opt.rnn_size) # we, fc, h^2_t-1
        self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2 + 1280, opt.rnn_size) # h^1_t, \hat v
        self.attention = Attention(opt)
        self.logit_embed = nn.Sequential(nn.Linear(15, 512),
                                      )
        self.softmax = nn.Softmax(1)

    def forward(self, xt, fc_feats, att_feats, p_att_feats, spatial_feats, logit, p_news_type_vector, state, att_masks=None):
        prev_h = state[0][-1]
        # pred = torch.argmax(logit, dim=1)
        # logit_pred = logit.new_zeros(logit.size(0), logit.size(1))
        # logit_pred = logit_pred.scatter_(1, pred.view(-1, 1), 1)
        pp_logit = self.logit_embed(self.softmax(logit))
        # if self.training:
        #     sample_prob = fc_feats.new(fc_feats.shape[0]).uniform_(0, 1)
        #     sample_mask = sample_prob > 0.4
        #     sample_ind = sample_mask.nonzero().view(-1)
        #     p_news_type_vector = p_news_type_vector[sample_ind]
        #     pp_logit.index_copy_(0, sample_ind, p_news_type_vector)

        att_lstm_input = torch.cat([prev_h, fc_feats, xt, pp_logit], 1)
        # att_lstm_input = torch.cat([prev_h, fc_feats, xt], 1)

        h_att, c_att = self.att_lstm(att_lstm_input, (state[0][0], state[1][0]))
        att = self.attention(h_att, att_feats, p_att_feats, att_masks)
        lang_lstm_input = torch.cat([att, h_att, spatial_feats], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], state[1][1]))
...
class News_Type_Class_Network(nn.Module):
    def __init__(self, opt):
        super(News_Type_Class_Network, self).__init__()
        self.fc_feat_size = opt.fc_feat_size
        self.rnn_size = opt.rnn_size
        self.drop_prob_lm = opt.drop_prob_lm
        ...
        # self.softmax = nn.Softmax(dim=1)
    def forward(self, fc_feats, spatial_feats, node_feats):
        # batch_size = fc_feats.size(0)
        # feats_size = spatial_feats.size(-1)
        # p_fc_feats = self.fc_embed_news(fc_feats)
        # p_node_feats = self.node_fc(node_feats)
        # spatial_feats = spatial_feats.reshape(batch_size, -1, feats_size)
        # spatial_feats_att = self.multi_head_self_attention(spatial_feats)
        # spatial_feats_att_ff = self.Feedforward(spatial_feats_att)
        # spatial_feats_att_ff = spatial_feats_att_ff.mean(1).squeeze()
        # p_node_feats = p_node_feats.mean(1).squeeze()
        # y = torch.cat((p_fc_feats, spatial_feats_att_ff, p_node_feats), dim=-1)
        # y = self.fc1(y)
        # y = self.fc2(y)
        # logit = self.fc3(y)
        out1 = self.fc1(spatial_feats)
        out2 = self.fc2(fc_feats)
        logit = self.fc3(torch.cat((out1, out2), dim=1))
...
class Attention(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats, att_masks=None):
        ...
        if att_masks is not None:
            weight = weight * att_masks.reshape(-1, att_size).to(weight)
            weight = weight / weight.sum(1, keepdim=True) # normalize to 1
        att_feats_ = att_feats.reshape(-1, att_size, att_feats.size(-1)) # batch * att_size * att_feat_size
...
class NewsAttModel(AttModel):
    def __init__(self, opt):
        super(NewsAttModel, self).__init__(opt)
        # version 1 : AdaAttCore
        # self.core = AdaAttCore(opt)
        # version 2 : updown
        self.num_layers = 2
...
class News_fusion_AttModel(AttModel):
    def __init__(self, opt):
        super(News_fusion_AttModel, self).__init__(opt)
        # version 1 : AdaAttCore
        # self.core = AdaAttCore(opt)
        # version 2 : updown
        self.num_layers = 2
        self.core = UpDown_fusion_Core(opt)


class show_attend_tell_Core(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(show_attend_tell_Core, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
        self.lstm = nn.LSTMCell(1000+1000+512, opt.rnn_size) # h^1_t, \hat v
        self.attention = Attention(opt)
        self.softmax = nn.Softmax(1)

    def forward(self, xt, fc_feats, att_feats, p_att_feats, spatial_feats, logit, p_news_type_vector, state, att_masks=None):
        prev_h = state[0][0]
        att = self.attention(prev_h, att_feats, p_att_feats, att_masks)
        att_lstm_input = torch.cat([prev_h,att,xt], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        h, c = self.lstm(att_lstm_input, (state[0][0], state[1][0]))
...
class show_tell_Core(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(show_tell_Core, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
        self.lstm = nn.LSTMCell(512, opt.rnn_size) # h^1_t, \hat v
        self.softmax = nn.Softmax(1)

    def forward(self, xt, fc_feats, att_feats, p_att_feats, spatial_feats, logit, p_news_type_vector, state, att_masks=None):
        prev_h = state[0][0]
        h, c = self.lstm(xt, (state[0][0], state[1][0]))
        output = F.dropout(h, self.drop_prob_lm, self.training)
        state = (h.unsqueeze(0),c.unsqueeze(0))
        return output, state

class News_type_fusion_AttModel(AttModel):
    def __init__(self, opt):
        super(News_type_fusion_AttModel, self).__init__(opt)
        # version 1 : AdaAttCore
        # self.core = AdaAttCore(opt)
        # version 2 : updown
        self.num_layers = 2
        self.core = UpDown_type_fusion_Core(opt)
        self.news_type_classify = News_Type_Class_Network(opt)
        self.news_type_embed = nn.Embedding(15, 512)
        self.q_linear = nn.Linear(opt.rnn_size, opt.rnn_size)
        self.k_linear = nn.Linear(opt.rnn_size, opt.rnn_size)
        self.d_k = opt.att_feat_size - 5
        self.gaussian_net = GaussianNet(opt)
        self.fc = nn.Sequential(nn.Linear(1280,512),
                                nn.ReLU(),
                                nn.Dropout(0.5))
