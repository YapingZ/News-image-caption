...
def getCOCO(dataset):
    if 'coco' in dataset:
        ...
        annFile = 'data/news_annotations/caption_test2017.json'
    return COCO(annFile)
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        tmp = [data['fc_feats'], data['att_feats'], data['spacial_feats'], data['labels'], data['masks'], data['att_masks'], data['news_type']]
        tmp = [_.to(device) if _ is not None else _ for _ in tmp]
        fc_feats, att_feats, spacial_feats, labels, masks, att_masks, news_type = tmp
        if labels is not None and verbose_loss:
            # forward the model to get loss
            with torch.no_grad():
                loss, loss_lang, loss_type = crit(model(fc_feats, att_feats,spacial_feats, labels[..., :-1], att_masks, news_type), news_type, labels[..., 1:], masks[..., 1:])
        ...
        with torch.no_grad():
            ...
            seq, seq_logprobs, logit = model(fc_feats, att_feats, spacial_feats, att_masks,  news_type, opt=tmp_eval_kwargs, mode='sample')
            seq = seq.data
            predict_news_type = logit.argmax(dim=1)
        ...
        acc += (predict_news_type ==  news_type).sum()
        news_type_list = news_type.cpu().numpy().tolist()
        predict_news_type_list = predict_news_type.cpu().numpy().tolist()
        for i, truth_type in enumerate(news_type_list):
            all_result_dict[truth_type] = all_result_dict.get(truth_type, 0 ) + 1
            if truth_type == predict_news_type_list[i]:
                result_dict[truth_type] = result_dict.get(truth_type, 0 ) + 1
        # Print beam search
        ...
        for k, sent in enumerate(sents):
            entry = {'image_id': data['infos'][k]['id'], 'caption': sent, 'perplexity': perplexity[k].item(), 'entropy': entropy[k].item(), 'predict': predict_news_type[k].item()}
    ...
    print("test news type accuracy: %f" % (acc / 2900))
    print(result_dict)
