...
def train(opt):
    ...
    ##########################
    if opt.noamopt:
        assert opt.caption_model in ['transformer', 'bert', 'm2transformer','mytransformer'], 'noamopt can only work with transformer'
    ...
    try:
        while True:
            ...
            optimizer.zero_grad()
            model_out = dp_lw_model(fc_feats, att_feats, spacial_feats, labels, masks, att_masks, news_type , data['gts'], torch.arange(0, len(data['gts'])), sc_flag, struc_flag)
            ...
            if model_out['loss_lang']:
                train_loss_lang = model_out['loss_lang'].item()
                train_loss_type = model_out['loss_type'].item()
            torch.cuda.synchronize()
            end = time.time()
            if struc_flag:
                ...
                print("iter {} (epoch {}), train_loss = {:.3f}, train_loss_lang = {:.3f}, train_loss_type = {:.3f}, time/batch = {:.3f}" \
                    .format(iteration, epoch, train_loss, train_loss_lang, train_loss_type, end - start))
            else:
